...
class Repository {
     ...
     */
    private function getRelationInstance($relation) {
        $class = "App\\".$relation;
    ...
    public function get(&$errors, $request) {
        ...
        $collection = $this->paginate($query, $request->paginate);
    ...
    /**
     * Includes relational data to the query. What's allowed to be included is set in the $relations variable.
     */
    public function with(&$query, &$errors, $withs) {
        ...
        foreach ($relations as $relation) {
            ...
            if (in_array($relation, $m_relations)) {
                ...
                array_push($with_errors, ["not_a_relation" => $relation]);
            }
        }

        // If with_errors is not empty, push it into errors-array
        if (!empty($with_errors))
            array_push($errors, ["with" => $with_errors]);
    }
     ...
     */
    public function paginate($query, $number) {
        ...
        if (isset($number) && is_numeric($number)) { 
            ...
            // Paginate and return result of query
